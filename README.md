...
Единственное, что может понадобиться — установленный **.NET 8 Desktop Runtime** (ссылка в шапке). На большинстве Windows 11 он уже присутствует, так что в типичном случае никаких дополнительных действий не требуется.
...
The only thing you may need is the **.NET 8 Desktop Runtime** (link at the top). On most Windows it's already there, so in the typical case no extra steps are required.